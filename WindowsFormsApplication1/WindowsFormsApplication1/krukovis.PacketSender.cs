...
namespace PWFrameWork
{
    ...
    public static class Packet
    {
        ...
        public static byte[] HostPlayerUseSkill(int skillID, int targetID)
        {
            ...
            byte[] packet = { 0x29, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0 };
        ...
        //Взять регистрацию
        public static byte[] GetRegistration()
        {
            byte[] packet = {0x9C};
            return packet;
        }
        ...
        public static byte[] Ref(int itemc, int itemid)
        {
            //02 00 EC 3D 10 80 - Выделить моба/НПС/Игрока. Песочный - WID моба/НПС или ID игрока. (c 2 по 5 = 4 байта - ID цели)
            //'2500230000000C000000'+itc+'0000'+itid+
            byte[] packet = {
                             0x25, 
                                0x00,
                                0x23,
            ...
            byte[] item_c = BitConverter.GetBytes(itemc);
            Array.Copy(item_c, 0, packet, 10, 2);
            byte[] item_id = BitConverter.GetBytes(itemid);
            Array.Copy(item_id, 0, packet, 14, 4);
            return packet;
        }
        public static byte[] Refgem(int itemc,int itemid,int gem)
        {
            //02 00 EC 3D 10 80 - Выделить моба/НПС/Игрока. Песочный - WID моба/НПС или ID игрока. (c 2 по 5 = 4 байта - ID цели)
            //'2500230000000C000000'+itc+'0000'+itid+
            byte[] packet = {
            ...
            byte[] item_gem = BitConverter.GetBytes(gem);
            Array.Copy(item_gem, 0, packet, 18, 2);      
            byte[] item_c = BitConverter.GetBytes(itemc);
            Array.Copy(item_c, 0, packet, 10, 2);
            byte[] item_id = BitConverter.GetBytes(itemid);
            Array.Copy(item_id, 0, packet, 14, 4);
        ...
        public static byte[] TransmitGhoToGenie(int GhoPointAmount)
        {
            byte[] packet = {
			0x71,
			0x0,
            ...
            byte[] ExpPoint_Amount = BitConverter.GetBytes(GhoPointAmount);
            Array.Copy(ExpPoint_Amount, 0, packet, 2, 4);
            return packet;
        }
        ...
        //Сдать квест
        //25 00 06 00 00 00 08 00 00 00 15 1B 00 00 00 00 00 00 
        public static byte[] AacceptQuest(int questID)
        {
            byte[] packet = {
			0x31, 0x00 ,0x03 ,0x00 ,0x00 ,0x00 ,0x01 ,0x00 ,0x00
		};
            byte[] quest_ID = BitConverter.GetBytes(questID);
            Array.Copy(quest_ID, 0, packet,7, 2);
            return packet;
        }

        public static byte[] DacceptQuest(int questID)
        {
            byte[] packet = {
			0x25,
			0x0,
